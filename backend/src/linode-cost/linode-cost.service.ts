...
@Injectable()
export class LinodeCostService {
  constructor(
  ...
  async getAccounts() {
    try {
      const list = await this.prisma.linodeAccount.findMany({
      ...
      if (msg.includes("doesn't exist") || msg.includes('Unknown table') || msg.includes('linode_accounts')) {
        throw new BadRequestException('Linode 相关数据库表未创建，请执行: npx prisma migrate deploy');
      }
      throw e;
    }
  }
  ...
  async updateCostQueryOrder(accountIds: number[]) {
    ...
    await this.prisma.linodeAccount.updateMany({ data: { costQueryEnabled: false } });
    for (let i = 0; i < ids.length; i++) {
      await this.prisma.linodeAccount.update({
        where: { id: ids[i] },
  ...
  async createAccount(data: { name: string; token: string; notes?: string }) {
    ...
    return this.prisma.linodeAccount.create({
      data: {
        name: data.name.trim(),
        token: encrypted,
  ...
  async updateAccount(
    ...
    return this.prisma.linodeAccount.update({ where: { id }, data: update });
  }

  async deleteAccount(id: number) {
    await this.prisma.linodeAccount.delete({ where: { id } });
    return { deleted: true };
  }
  ...
  async getCosts(year?: number, month?: number, accountId?: number) {
    try {
      ...
      return this.prisma.linodeCost.findMany({
        where,
      ...
      const msg = e instanceof Error ? e.message : String(e);
      if (msg.includes("doesn't exist") || msg.includes('Unknown table') || msg.includes('linode')) {
        throw new BadRequestException('Linode 相关数据库表未创建，请执行: npx prisma migrate deploy');
      }
      throw e;
    }
  ...
  async createCost(data: { year: number; month: number; accountId: number; project?: string; usage?: string; amount: number }) {
    return this.prisma.linodeCost.create({
      data: {
        year: data.year,
        month: data.month,
  ...
  async updateCost(id: number, data: { project?: string; usage?: string; amount?: number }) {
    ...
    return this.prisma.linodeCost.update({
      where: { id },
      data: update,
      include: { account: { select: { id: true, name: true } } },
    });
  ...
  async deleteCost(id: number) {
    await this.prisma.linodeCost.delete({ where: { id } });
    return { deleted: true };
  }
}
