...
@Controller('api/do-costs')
export class DoCostController {
  constructor(
    private readonly prisma: PrismaService,
    private readonly doCostService: DoCostService,
    private readonly doDailyCostService: DoDailyCostService,
  ) {}

  @Get('accounts')
  getAccounts() {
    return this.doCostService.getAccounts();
  }

  /** 调试用：获取指定账号的 DO Balance API 原始响应（用于排查费用解析问题） */
  @Get('debug/balance/:accountId')
  async getDebugBalance(@Param('accountId', ParseIntPipe) accountId: number) {
    return this.doDailyCostService.getRawBalanceResponse(accountId);
  }
  ...
  createAccount(@Body() body: { name: string; token: string; notes?: string }) {
    ...
    return this.doCostService.createAccount({
      name: body.name.trim(),
  ...
  updateCostQueryOrder(@Body() body: { accountIds: number[] }) {
    return this.doCostService.updateCostQueryOrder(body.accountIds ?? []);
  }

  @Put('accounts/:id')
  updateAccount(
    @Param('id', ParseIntPipe) id: number,
    @Body() body: { name?: string; token?: string; notes?: string; costQueryEnabled?: boolean; costQuerySortOrder?: number },
  ) {
    return this.doCostService.updateAccount(id, body);
  }

  @Delete('accounts/:id')
  removeAccount(@Param('id', ParseIntPipe) id: number) {
    return this.doCostService.deleteAccount(id);
  }
  ...
  async queryDailyCosts(@Body() body: { startDate: string; endDate: string; accountIds?: number[] }) {
    ...
    return Promise.all(
      ids.map((accountId) =>
        this.doDailyCostService.queryAndSaveOne({ startDate, endDate, accountId }),
      ),
    );
  }
  ...
  getMonthTotal(
    ...
    return this.doDailyCostService.getMonthTotal({
      year,
      month,
  ...
  async syncDailyToCost(@Body() body?: { year?: number; month?: number; accountIds?: number[] }) {
    ...
    await this.doDailyCostService.syncMonthlyToCostRegistration({ year, month, accountIds });
    return { success: true };
  }
  ...
  updateCost(
    ...
    return this.doCostService.updateCost(id, body);
  }

  @Delete(':id')
  deleteCost(@Param('id', ParseIntPipe) id: number) {
    return this.doCostService.deleteCost(id);
  }
}
