...
import { Plus, Trash2, Pencil, X, Cpu, ChevronLeft, ChevronRight, Calendar, Search } from 'lucide-react';
import { api, type LinodeAccount, type LinodeCost } from '../api';
...
function DailyCostQueryPanel({ accounts, load, toast }: {
  ...
  const doQuery = async () => {
    ...
    try {
      for (let i = 0; i < accountIds.length; i++) {
        ...
        await api.queryLinodeDailyCosts({ startDate: today, endDate: today, accountIds: [id] });
      }
  ...
  const loadMonthTotal = () => {
    ...
    api
      .getLinodeMonthTotal({ year, month, accountIds: accountIds.length > 0 ? accountIds : undefined })
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          ...
          <p className="text-sm text-slate-500 mt-1">Linode API 仅提供当月至今（MTD）累计费用，每天查询一次并记录当日增量。点击查询将拉取今日 MTD 并计算今日费用保存。每天 19:40 自动执行。</p>
        </div>
...
export default function LinodeCost() {
  const now = new Date();
  ...
  const [costs, setCosts] = useState<LinodeCost[]>([]);
  const [projects, setProjects] = useState<{ id: number; name: string }[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [accountModal, setAccountModal] = useState<'new' | 'edit' | null>(null);
  const [editingAccount, setEditingAccount] = useState<LinodeAccount | null>(null);
  const [accountForm, setAccountForm] = useState({ name: '', token: '', notes: '' });
  const [accountSaving, setAccountSaving] = useState(false);
  const [accountDeleteConfirm, setAccountDeleteConfirm] = useState<LinodeAccount | null>(null);

  const [costModal, setCostModal] = useState<'new' | 'edit' | null>(null);
  const [editingCost, setEditingCost] = useState<LinodeCost | null>(null);
  const editingCostRef = useRef<LinodeCost | null>(null);
  ...
  const [costDeleteConfirm, setCostDeleteConfirm] = useState<LinodeCost | null>(null);

  const load = () => {
    setLoading(true);
    setError(null);
    Promise.all([
      api.getLinodeAccounts(),
      api.getLinodeCosts(year, month),
      api.getProjects(),
    ])
  ...
  const openEditAccount = (acc: LinodeAccount) => {
  ...
  const handleSaveAccount = async () => {
    ...
    try {
      if (accountModal === 'new') {
        await api.createLinodeAccount({ name, token: accountForm.token.trim(), notes: accountForm.notes?.trim() || undefined });
        toast.success('账号已添加');
      } else if (editingAccount) {
        await api.updateLinodeAccount(editingAccount.id, {
          name,
  ...
  const doDeleteAccount = async () => {
    ...
    try {
      await api.deleteLinodeAccount(accountDeleteConfirm.id);
      load();
  ...
  const toggleCostQuery = async (acc: LinodeAccount) => {
    try {
      await api.updateLinodeAccount(acc.id, { costQueryEnabled: !acc.costQueryEnabled });
      load();
  ...
  const openEditCost = (row: LinodeCost) => {
  ...
  const handleSaveCost = async () => {
    ...
    try {
      if (costModal === 'new') {
        ...
        await api.updateLinodeCost(editing.id, {
          project: (costForm.project ?? '').trim(),
          usage: (costForm.usage ?? '').trim() || undefined,
  ...
  const handleSyncDailyToCost = async () => {
    ...
    try {
      await api.syncLinodeDailyToCost({ year, month });
  ...
  const doDeleteCost = async () => {
    ...
    try {
      await api.deleteLinodeCost(costDeleteConfirm.id);
      load();
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              {activeTab === 'accounts' ? '管理 Linode 账号，配置 Token 用于费用查询' : activeTab === 'costs' ? '在各账号下登记每月费用' : '查询 MTD 费用并同步到费用登记'}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200">
              {[
                ...
                { id: 'dailyQuery' as const, label: 'Linode每日费用查询' },
              ].map((t) => (
                <button
                  key={t.id}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 overflow-y-auto" onClick={() => setAccountModal(null)}>
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md my-8" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <h2 className="text-lg font-bold text-slate-900">{accountModal === 'new' ? '添加 Linode 账号' : '编辑账号'}</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={accountForm.name} onChange={(e) => setAccountForm((f) => ({ ...f, name: e.target.value }))} placeholder="如 optimizenet" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>API Token {accountModal === 'edit' && '(留空不修改)'}</label>
                <input type="password" value={accountForm.token} onChange={(e) => setAccountForm((f) => ({ ...f, token: e.target.value }))} placeholder="Personal Access Token" className={inputClass} />
              </div>
              <div>
